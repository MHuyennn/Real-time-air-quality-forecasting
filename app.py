...
with col3:
    ...
    with tab1:
        ...
        if highlight_location != 'Tất cả':
            ...
            # Tải mô hình
            exp = load_model(r"D:\This\CS\AQI-Globe\AQI-Globe\softs.pt")
        ...
        if highlight_location != 'Tất cả':
            if highlight_location != 'Tất cả':
                ...
                if st.button("Dự đoán", key="health_predict"):
                    duration_factor = min(outdoor_time * 0.6, 5.0)
                    activity_mapping = {
                        "Nghỉ ngơi": 1.0,
                    ...
                    health_condition_mapping = {
                        "Không có": 0,
                        "Bệnh liên quan đến hô hấp": 0.5,
                        "Bệnh liên quan đến tim mạch": 0.5,
                        "Cả hai": 1.0
                    }
                    sensitivity_factor += health_condition_mapping[health_condition]
                    ...
                    if risk_score < 0.25:
                        ...
                        risk_icon = "✅"
                    elif 0.25 <= risk_score <= 0.6:
                        risk_description = "Hạn chế ra ngoài nếu không cần thiết"
                        risk_icon = "⚠️"
                    elif 0.6 < risk_score <= 1.0:
                        risk_description = "Khuyến nghị tránh ra ngoài trong thời gian dài, cân nhắc ở trong nhà"
                        risk_icon = "🚫"
                    else:
                        risk_description = "Cảnh báo nghiêm trọng, nên ở trong nhà và tìm kiếm tư vấn bác sĩ"
                        risk_icon = "🛑"

                    st.markdown(
                        f"""
                        <div class="recommendation-box">
                            {risk_icon} {risk_description}<br>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
